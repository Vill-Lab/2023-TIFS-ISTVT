...
class EnsembleEfficientNet(nn.Module):
    def __init__(self,num_nets = 3):
        ...
        for _ in range(num_nets):
            self.nets.append(EfficientNet.from_pretrained('efficientnet-b2', num_classes = 2))
...
class EnsembleXceptionNet(nn.Module):
    def __init__(self,num_nets = 3):
        super(EnsembleXceptionNet,self).__init__()
        self.nets = nn.ModuleList()
...
class JigsawSolver(nn.Module):
    def __init__(self,out_count,in_channels = 1408):
        ...
        self.pool = nn.AdaptiveAvgPool2d(output_size = 1)
        self.fc1 = nn.Linear(in_channels,1408)
        self.fc2 = nn.Linear(1408,1408)
        self.fc3 = nn.Linear(1408,704)
        self.fc4 = nn.Linear(704,out_count * 2)
        self.bn1 = nn.BatchNorm2d(1408)
        self.bn2 = nn.BatchNorm2d(1408)
        self.bn3 = nn.BatchNorm2d(704)
        self.bn4 = nn.BatchNorm2d(out_count * 2)
...
class JigsawSolverConv(nn.Module):
    def __init__(self,out_count,in_channels = 1408):
        ...
        self.bn2 = nn.BatchNorm1d(out_count * 2)
...
class EnsembleEN(nn.Module):
    def __init__(self,num_nets = 3):
        ...
        self.pool = nn.AdaptiveAvgPool2d(output_size = 1)
        for _ in range(num_nets):
            self.nets.append(EfficientNet.from_pretrained('efficientnet-b4', num_classes = 0))
            self.fcs.append(nn.Linear(1792,2))
...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes=2048, ratio=16):
...
class VaniTripleXcep(nn.Module):
    def __init__(self,num_nets = 3):
        super(VaniTripleXcep,self).__init__()
        self.nets = nn.ModuleList()
        self.fcs = nn.ModuleList()
        for _ in range(num_nets):
            self.nets.append(get_xception())
...
class EnsembleXcep(nn.Module):
    def __init__(self,num_nets = 3):
        ...
        self.nets = nn.ModuleList()
        self.fcs = nn.ModuleList()
        self.chan_attns = nn.ModuleList()
        self.attn_nets = nn.ModuleList()
        self.pool = nn.AdaptiveAvgPool2d(output_size = 1)
        self.bns = nn.ModuleList()
        for _ in range(num_nets):
            self.nets.append(get_xception())
            self.fcs.append(nn.Linear(2048,1))
            self.attn_nets.append(SpatialAttention())
            self.chan_attns.append(ChannelAttention())
            self.bns.append(nn.BatchNorm1d(2))
        self.dropout = nn.Dropout(0.5)
        #self.attn_gen = SpatialAttention()
        #self.weight_gen = SelfAttentionBranchWeightGenerator()
        #self.final_fc = nn.Linear(6144,2)
    ...
    def forward(self,inputs):
        ...
        for (x,net,fc,s_attn_gen,c_attn_gen,bn) in zip(inputs,self.nets,self.fcs,self.attn_nets,self.chan_attns,self.bns):
                feat = net.features(x)
                #c_attn = c_attn_gen(feat)
                #feat = feat * c_attn
                s_attn = s_attn_gen(feat)
                feat = feat * s_attn
                feats.append(feat)
        ...
        return clas,feats,s_attns,[]
        #branch weights

        #batch_len = len(inputs[0])
        #weights = self.weight_gen(feats)
        #cla_fusion = clas[0].detach()*weights[:,0].view(batch_len,1) + clas[1].detach()*weights[:,1].view(batch_len,1) + clas[2].detach()*weights[:,2].view(batch_len,1)
        #cla_fusion = clas[0] + clas[1] + clas[2]
        #feat_fusion = torch.cat((feats[0], feats[1], feats[2]),dim=1)
        #cla_fusion = self.final_fc(self.dropout(self.pool(feat_fusion).flatten(start_dim = 1)))

        #return clas,feats,cla_fusion,weights
...
class BranchWeightGenerator(nn.Module):
    def __init__(self):
        ...
        #self.cnn = EfficientNet.from_pretrained('efficientnet-b2',num_classes=0)
...
class SelfAttentionBranchWeightGenerator(nn.Module):
    def __init__(self):
        super(SelfAttentionBranchWeightGenerator, self).__init__()
        #self.cnn = EfficientNet.from_pretrained('efficientnet-b2',num_classes=0)
        self.q_projection = nn.Linear(2048,512)
        self.k_projection = nn.Linear(2048,512)
        self.v_projection = nn.Linear(2048,512)
        self.d_k = 512 ** 0.5
        self.fc = nn.Linear(512*3,3)
        self.pool = nn.AdaptiveAvgPool2d(output_size=1)
    def forward(self, feats):
        q=[]
        k=[]
        ...
        for f in feats:
            ...
            q.append(self.q_projection(f))
            k.append(self.k_projection(f))
            v.append(self.v_projection(f))
        Q = torch.cat((q[0],q[1],q[2]),dim = 1).view(-1,3,512)
        K = torch.cat((k[0],k[1],k[2]),dim = 1).view(-1,3,512)
        V = torch.cat((v[0],v[1],v[2]),dim = 1).view(-1,3,512)
        self_att = F.softmax((torch.matmul(Q,K.transpose(-2,-1))/self.d_k), dim = -1)
        V_att = torch.matmul(self_att,V)


        return torch.softmax(self.fc(V_att.flatten(1)),1)