...
def recombine_features(gs,ids,imgs,label):
    ...
    img_seq_r = torch.cat((imgs[0][label==0],imgs[2][label==1]),0)
    img_seq_fr = torch.cat((imgs[2][label==0],imgs[0][label==1]),0)
    
    return g_real,id_fake_r,g_fake_r,id_real,img_seq_fr,img_seq_r
...
def get_same_padding_conv2d(image_size=None,use_cdc = False):
    ...
    """
    t = 0.7 if use_cdc else 0
    if image_size is None:
        return partial(Conv2dDynamicSamePadding, theta = t)
    else:
        return partial(Conv2dStaticSamePadding, image_size=image_size, theta = t)
...
class Conv2dDynamicSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, bias=True,theta = 0):
    ...
    def forward(self, x):
        ...
        return F.conv2d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)# - self.theta * out_diff
...
class Conv2dStaticSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None,theta = 0, **kwargs):
    ...
    def forward(self, x):
        ...
        #kernel_diff = self.weight.sum(2).sum(2)
        #kernel_diff = kernel_diff[:, :, None, None]
        #out_diff = F.conv2d(x, kernel_diff, self.bias, self.stride, 0, self.dilation, self.groups)
