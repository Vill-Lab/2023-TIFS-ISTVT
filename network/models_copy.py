...
class TransferModel(nn.Module):
    ...
    def __init__(self, modelchoice, num_out_classes=2, dropout=0.5, batch_size=16):
        ...
        if modelchoice in ['xception']:
            ...
            #self.model = resnet.i3_res50_nl(num_classes = 1)
    ...
    def features(self,x):
        return F.adaptive_avg_pool2d(self.model.features(x),1).view(-1,2048)
    
    def feature_maps(self,x):
        return self.model.features(x)

    def low_level_features(self,x):
        return self.model.low_level_features(x)
